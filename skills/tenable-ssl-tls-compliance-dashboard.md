...
encryption and certificate compliance. It's aimed at infrastructure and security
...
- Surfaces certificates that are expired, self-signed, expiring soon, or signed
  with a weak algorithm/key (SHA-1, MD5, RSA < 2048 bits).
...
or restarts services — recommendations are text, not actions. With no active MCP
connection it falls back to a clearly labeled DEMO dashboard.
